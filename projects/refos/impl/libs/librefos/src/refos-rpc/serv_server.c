...
void server_serv_connect_direct(void *rpc_userptr) {
    ...
    int* rpc_errno = rpc_malloc(sizeof(int));
    seL4_CPtr rpc___ret__ = 0;
    rpc_sv_track_obj(rpc_userptr, rpc_errno);


    rpc___ret__ = serv_connect_direct_handler(rpc_userptr, rpc_liveness, rpc_errno);

    reply_serv_connect_direct(rpc_userptr, rpc_errno, rpc___ret__);
}

void reply_serv_connect_direct(void *rpc_userptr, int* rpc_errno, seL4_CPtr rpc___ret__) {
    rpc_reset_contents(rpc_userptr);
    rpc_sv_push_buf(rpc_userptr, rpc_errno, sizeof(int));
